...
theta2_reg = theta1.copy()

from scipy import interpolate


fill_vals = (A - r1[0], A - r1[-1])
f_r2 = interpolate.interp1d(theta2_scattered, A - r1, bounds_error=False, fill_value=fill_vals)
r2_reg = f_r2(theta2_reg)

fill_vals = (theta2_scattered[0], theta2_scattered[-1])
f_t2 = interpolate.interp1d(theta1, theta2_scattered, bounds_error=False, fill_value=fill_vals)
...
for n, offset1 in enumerate ( np.linspace(0, 2*np.pi, n_step) ):

    # ix = range(k,n_pts) + range(0,k)

    x = r1 * np.cos(theta1 + offset1)
    y = r1 * np.sin(theta1 + offset1)
    ...
    offset2 = f_t2(offset1)

    x2 = A - r2_reg * np.cos(theta2_reg + offset2)
    y2 =   r2_reg * np.sin(theta2_reg + offset2)
    ...
    plt.savefig('frames/f_{:04d}.png'.format(n))
